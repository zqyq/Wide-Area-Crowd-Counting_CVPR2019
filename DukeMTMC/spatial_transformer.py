...
class SpatialTransformer(Layer):
    ...
    def _transform(self, view, input_shape, output_size):
        ...
        if (view == 1):
            view1_ic = np.load(
                'coords_correspondence_DukeMTMC/projection_forth/view1_correspondence_forth_fullHeight.npz')
            ...
            view1_gp_mask = cv2.resize(view1_gp_mask, (int(640 / 4), int(480 / 4)))
            ...
            view_norm_mask = np.load('coords_correspondence_DukeMTMC/norm/view_Wld_normalization_forth_mask.npz')
            ...
            view2_ic = np.load(
                'coords_correspondence_DukeMTMC/projection_forth/view2_correspondence_forth_fullHeight.npz')
            ...
            view2_gp_mask = cv2.resize(view2_gp_mask, (int(640 / 4), int(480 / 4)))
            ...
            view3_ic = np.load(
                'coords_correspondence_DukeMTMC/projection_forth/view3_correspondence_forth_fullHeight.npz')
            ...
            view3_gp_mask = cv2.resize(view3_gp_mask, (int(640 / 4), int(480 / 4)))
            ...
            view4_ic = np.load(
                'coords_correspondence_DukeMTMC/projection_forth/view4_correspondence_forth_fullHeight.npz')
            view4_ic = view4_ic.f.arr_0
            view4_ic = tf.cast(view4_ic, 'float32')
            view4_ic = tf.expand_dims(view4_ic, axis=0)
            view4_ic = tf.tile(view4_ic, [batch_size, 1, 1])

            transformed_grid = tf.cast(view4_ic, 'float32')

            view4_gp_mask = np.load('coords_correspondence_DukeMTMC/mask/view4_GP_mask.npz')
            view4_gp_mask = view4_gp_mask.f.arr_0
            view4_gp_mask = cv2.resize(view4_gp_mask, (int(640 / 4), int(480 / 4)))
            view4_gp_mask = tf.cast(view4_gp_mask, 'float32')
            view4_gp_mask = tf.expand_dims(view4_gp_mask, axis=0)
            view4_gp_mask = tf.expand_dims(view4_gp_mask, axis=3)
            view4_gp_mask = tf.tile(view4_gp_mask, [batch_size, 1, 1, num_channels])
            view_gp_mask = tf.cast(view4_gp_mask, 'float32')

            view_norm_mask = np.load('coords_correspondence_DukeMTMC/norm/view_Wld_normalization_forth_mask.npz')
            view4_norm_mask = view_norm_mask.f.arr_0[2]
            view4_norm_mask = tf.cast(view4_norm_mask, 'float32')
            view4_norm_mask = tf.expand_dims(view4_norm_mask, axis=0)
            view4_norm_mask = tf.expand_dims(view4_norm_mask, axis=3)
            view4_norm_mask = tf.tile(view4_norm_mask, [batch_size, 1, 1, num_channels])
            view_norm_mask = tf.cast(view4_norm_mask, 'float32')
